...
for link in shareable_links:
    with st.container():
        ...
        if link.get("data_key") and link["data_key"] in st.session_state:
            ...
            if data:
                with st.expander(
                    f"📊 View {link['name']} Responses ({len(data)} total)"
                ):
                    # Quick stats
                    if link["data_key"] == "survey_responses":
                        ...
                        if st.checkbox(
                            f"Show all responses", key=f"show_all_{link['name']}"
                        ):
                            for i, resp in enumerate(data, 1):
                                st.write(
                                    f"**{i}.** {resp['name']} - {resp['response']} ({resp['timestamp']})"
                                )

                    # Clear data button
                    if st.button(
                        f"�️ Clear Data", key=f"clear_{link['name']}", type="secondary"
                    ):
