...
# Fetch sales inventory details from the database
st.title("View/Edit Sales Inventory")
sales_inventory = fetch_inventory_details("sale_inventory")

if sales_inventory:
    # Convert the sales inventory data to a DataFrame
    df = pd.DataFrame(sales_inventory)
    ...
    edited_df = st.data_editor(
        ...
        key="sales_inventory_data_editor",
        height=500,
    ...
    def update_column(column_name, new_value, item_id):
        query = f"UPDATE sale_inventory SET {column_name} = '{new_value}' WHERE item_id = '{item_id}'"
    ...
    st.error("No sales inventory data available.")