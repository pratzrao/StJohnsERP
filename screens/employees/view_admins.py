...
import pandas as pd
from services.db_helper import fetch_admin_details, get_connection

# Fetch admin details from the database
st.title("View/Edit Admin Details")
admins = fetch_admin_details()

if admins:
    # Convert the admin data to a DataFrame
    df = pd.DataFrame(admins)

    # Display the data editor table with the 'admin_id' column disabled (non-editable)
    edited_df = st.data_editor(
        df,
        disabled=("admin_id",),  # Disable the admin_id column (non-editable)
    ...
    for index, original_row in df.iterrows():
        ...
        if not original_row.equals(edited_row):  
            ...
            rows_to_update.append((edited_row, original_row['admin_id']))

    # Update function using f-strings for SQL queries
    def update_column(column_name, new_value, admin_id):
        query = f"UPDATE admin_details SET {column_name} = '{new_value}' WHERE admin_id = '{admin_id}'"
    ...
    if st.button("Save Changes"):
        ...
        for edited_row, admin_id in rows_to_update:
            ...
            for column_name, new_value in edited_row.items():
                if new_value != df.at[edited_row.name, column_name]:  # Check if value is changed
                    update_column(column_name, new_value, admin_id)  # Update the column in the database
        st.success("Changes saved successfully.")
else:
    st.error("No admin data available.")